...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.provision "chef_solo" do |chef|
    ...
    chef.add_recipe "set_time_zone"
    chef.add_recipe "login_user_env"
    ...
    chef.json = {
      ...
      "login_user_env" => {
        'user_name' => 'vagrant',
        'env_list' => {
            'TZ' => %q(`ls -l /etc/localtime | sed -n 's/.*\\/zoneinfo\\/\\(.*\\)/\\1/p'`),
            'VILLE_SERV_CONF' => 'local_dev',
            'MHVILLE_SERVER_API_PORT' => '80'
          }
      }
    }
  end
